...
router.post("/orders", async (req, res: Response) => {
  ...
  if (body.side === "buy") {
    ...
    if (existing) {
      ...
      const newAvg = newQty > 0 ? (prevAvg * prevQty + price * body.quantity) / newQty : 0;
      await db
        .update(holdings)
        .set({
    ...
    if (newQty <= 0) {
      ...
      await db
        .update(holdings)
        .set({ quantity: newQty.toFixed(6), updatedAt: new Date() })
        .where(eq(holdings.id, existing!.id));
    }
    await db
      .update(accounts)
  ...
  await db.insert(transactions).values({
    userId,
    type: body.side,
    description: `${body.side === "buy" ? "Bought" : "Sold"} ${body.quantity} ${symbol} @ $${price.toFixed(2)}`,
