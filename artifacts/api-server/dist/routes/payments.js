...
router.post("/deposit", async (req, res) => {
    ...
    if (!stripe) {
        res.status(503).json({
            error: "Stripe is not configured yet. Add the Stripe integration to enable account funding.",
        });
        return;
    }
...
router.post("/confirm", async (req, res) => {
    ...
    const account = (await db.select().from(accounts).where(eq(accounts.userId, userId)))[0];
    if (!account) {
