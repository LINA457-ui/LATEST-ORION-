...
// ─── PIN management ──────────────────────────────────────────────────────
router.get("/pins", requirePinVerified, async (_req, res) => {
    const rows = await db
        .select({
        id: adminPins.id,
...
router.post("/pins", requirePinVerified, async (req, res) => {
    const userId = userIdOf(req);
    ...
    try {
        const created = await db.transaction(async (tx) => {
            ...
            if (Number(existing) >= MAX_PINS) {
                throw new Error(`Maximum of ${MAX_PINS} PINs allowed. Remove one before adding a new one.`);
            }
            const [row] = await tx
                .insert(adminPins)
                .values({
        ...
        const msg = e.message ?? "Failed to add PIN";
        if (msg.includes("admin_pins_pin_hash_unique") ||
...
router.patch("/pins/:id", requirePinVerified, async (req, res) => {
...
router.delete("/pins/:id", requirePinVerified, async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isFinite(id)) {
        res.status(400).json({ error: "Invalid PIN id" });
        return;
...
router.get("/overview", async (_req, res) => {
    ...
    const signupsToday = allAccounts.filter((a) => now - a.createdAt.getTime() < dayMs).length;
    const signupsThisWeek = allAccounts.filter((a) => now - a.createdAt.getTime() < 7 * dayMs).length;
    ...
    res.json({
        ...
        adminUsers: allAccounts.filter((a) => a.isAdmin)
            .length,
...
router.get("/users/:userId", async (req, res) => {
    ...
    res.json({
        account: {
            ...
            displayedTotalEquity: accountRow.equityOverride != null
                ? Number(accountRow.equityOverride)
                : computedTotal,
            displayedPortfolioValue: accountRow.marketValueOverride != null
                ? Number(accountRow.marketValueOverride)
                : +portfolioValue.toFixed(2),
            displayedBuyingPower: accountRow.buyingPowerOverride != null
                ? Number(accountRow.buyingPowerOverride)
                : cash,
            displayedDayChange: accountRow.dayChangeOverride != null
                ? Number(accountRow.dayChangeOverride)
                : null,
            displayedDayChangePercent: accountRow.dayChangePercentOverride != null
                ? Number(accountRow.dayChangePercentOverride)
                : null,
            overrides: {
                equity: accountRow.equityOverride != null
                    ? Number(accountRow.equityOverride)
                    : null,
                marketValue: accountRow.marketValueOverride != null
                    ? Number(accountRow.marketValueOverride)
                    : null,
                buyingPower: accountRow.buyingPowerOverride != null
                    ? Number(accountRow.buyingPowerOverride)
                    : null,
                dayChange: accountRow.dayChangeOverride != null
                    ? Number(accountRow.dayChangeOverride)
                    : null,
                dayChangePercent: accountRow.dayChangePercentOverride != null
                    ? Number(accountRow.dayChangePercentOverride)
                    : null,
            },
        },
...
router.patch("/users/:userId/cash", async (req, res) => {
    const { userId } = req.params;
    ...
    await db.transaction(async (tx) => {
        ...
        if (delta !== 0) {
            await tx.insert(transactions).values({
                ...
                description: note && typeof note === "string" && note.trim().length > 0
...
router.patch("/users/:userId/overrides", async (req, res) => {
    const { userId } = req.params;
    const body = (req.body ?? {});
    function toCol(value, scale) {
        if (value === undefined)
            return undefined;
        if (value === null)
            return null;
    ...
    const updates = {};
    if (equityCol !== undefined)
        updates.equityOverride = equityCol;
    if (marketCol !== undefined)
        updates.marketValueOverride = marketCol;
    if (buyingCol !== undefined)
        updates.buyingPowerOverride = buyingCol;
    if (changeCol !== undefined)
        updates.dayChangeOverride = changeCol;
    if (pctCol !== undefined)
        updates.dayChangePercentOverride = pctCol;
    if (Object.keys(updates).length === 0) {
...
router.post("/users/:userId/holdings", async (req, res) => {
    const { userId } = req.params;
...
router.delete("/users/:userId/holdings/:symbol", async (req, res) => {
    const { userId, symbol } = req.params;
    await db
        .delete(holdings)
        .where(and(eq(holdings.userId, userId), eq(holdings.symbol, symbol.toUpperCase())));
    res.json({ ok: true });
});
...
router.post("/users/:userId/transactions", async (req, res) => {
    const { userId } = req.params;
    const { type, description, amount, symbol, createdAt } = (req.body ?? {});
    ...
    const sanitizedSymbol = typeof symbol === "string" && symbol.trim().length > 0
...
router.get("/orders", async (_req, res) => {
    ...
    const accountRows = userIds.length
        ...
        : [];
    const userMap = new Map(accountRows.map((a) => [
        String(a.userId),
        {
