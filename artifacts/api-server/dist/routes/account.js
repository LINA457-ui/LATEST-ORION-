...
export async function getAccountSnapshot(userId) {
    ...
    const computedDayChangePercent = previousValue > 0
        ? Number(((computedDayChange / previousValue) * 100).toFixed(2))
        : 0;
    const portfolioValueDisplay = account.marketValueOverride != null
        ? Number(account.marketValueOverride)
        : computedPortfolioValue;
    const totalEquityDisplay = account.equityOverride != null
        ? Number(account.equityOverride)
        : computedTotalEquity;
    const buyingPowerDisplay = account.buyingPowerOverride != null
        ? Number(account.buyingPowerOverride)
        : cashBalance;
    const dayChangeDisplay = account.dayChangeOverride != null
        ? Number(account.dayChangeOverride)
        : computedDayChange;
    const dayChangePercentDisplay = account.dayChangePercentOverride != null
    ...
    return {
        ...
        overrides: {
            ...
            marketValue: account.marketValueOverride != null
                ? Number(account.marketValueOverride)
                : null,
            buyingPower: account.buyingPowerOverride != null
                ? Number(account.buyingPowerOverride)
                : null,
            dayChange: account.dayChangeOverride != null
                ? Number(account.dayChangeOverride)
                : null,
            dayChangePercent: account.dayChangePercentOverride != null
                ? Number(account.dayChangePercentOverride)
                : null,
        },
    };
...
router.post("/transactions", async (req, res) => {
    const userId = userIdOf(req);
    const { type, description, amount, symbol } = (req.body ?? {});
...
router.get("/dashboard", async (req, res) => {
    ...
    const positions = userHoldings
        ...
        const unrealizedPnl = Number((quantity * (quote.price - averageCost)).toFixed(2));
        const unrealizedPnlPercent = averageCost > 0
