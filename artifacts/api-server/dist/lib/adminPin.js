...
import { adminPins } from "@workspace/db/schema";
import { count } from "drizzle-orm";
function requireSecret() {
    const s = process.env.SESSION_SECRET;
    if (!s || s.length < 16) {
        throw new Error("SESSION_SECRET environment variable is required (min 16 chars) for admin PIN security.");
    }
    return s;
}
...
export async function verifyPin(pin) {
    ...
    for (const r of rows) {
        if (r.pinHash.length === target.length &&
...
export function issuePinToken(userId) {
...
export function verifyPinToken(token, userId) {
